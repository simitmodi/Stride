...
export default function BankPastAppointments() {
    ...
    useEffect(() => {
        ...
        const fetchAppointments = async () => {
            ...
            try {
                // Here we just fetch appointments that have been handled by this bank user.
                // Assuming the bank user has a unique ID associated with their actions or we just fetch appointments for their bank/branch.
                // Since we don't have a reliable way to know the bank user's bankName/branch just from user ID in this component easily without fetching their user doc,
                // we'll fetch all past appointments for the bank they belong to.

                // Let's first fetch the bank user's details to know their bankName and branch
        ...
        fetchAppointments();
    }, [user, isUserLoading, appointmentsRef, firestore]);
                                        ...
                                        <CardContent className="p-4 flex justify-between items-center gap-4">
                                            <div className="flex-1">
                                                ...
                                                <CardDescription className="text-foreground/80">
                                                    {format(apt.date.toDate(), 'PPP')} at {apt.time}
                                                </CardDescription>
                                            </div>
                                            {apt.deleted && (
